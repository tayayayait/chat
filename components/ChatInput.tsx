...
const ChatInput: React.FC<ChatInputProps> = ({ onSendMessage, onStop, isLoading }) => {
    ...
    <div className="px-4 py-3 bg-gray-900/50 border-t border-gray-700/50 backdrop-blur-sm">
      <form onSubmit={handleSubmit} className="flex items-end gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isLoading ? (
              <span className="w-5 h-5 border-2 border-white/70 border-t-transparent rounded-full animate-spin" aria-hidden="true" />
            ) : (
              <SendIcon />
            )}
          </button>
          {isLoading && onStop && (
            <button
              type="button"
              onClick={onStop}
              className="px-3 py-2 text-sm font-medium bg-gray-700 text-white rounded-md hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-gray-800"
            >
              중지
            </button>
          )}
