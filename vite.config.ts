...
const createChatMiddleware = (apiKey?: string) => {
...
const registerGeminiProxy = (middlewares: any, apiKey?: string) => {
  middlewares.use('/api/chat', createChatMiddleware(apiKey));
};

const geminiProxyPlugin = (apiKey?: string) => ({
  name: 'gemini-chat-proxy',
  configureServer(server: any) {
    registerGeminiProxy(server.middlewares, apiKey);
  },
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [react(), geminiProxyPlugin(apiKey)],
    resolve: {
      alias: {
